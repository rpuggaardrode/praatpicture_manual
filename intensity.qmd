...
::: {.callout-warning}
The option of including a background color for intensity contours when overlaying on a spectrogram is a recent addition to the library, and it is not yet possible in the version of `praatpicture` available on CRAN. You have to download the development version if you would like to use this.
:::
